...
var post_stats = function elastic_post_stats(statString) {
  if (elasticHost) {
    try {
      ...
      var indexMo = indexDate.getMonth() +1;
      if (indexMo < 10) {
          indexMo = '0'+indexMo;
      }
      var indexDt = indexDate.getDate();
      if (indexDt < 10) {
          indexDt = '0'+indexDt;
      }

      var statsdIndex = elasticIndex + '-' + indexDate.getFullYear() + '.' + indexMo + '.' +  indexDt;
      elasticUrl = 'http://' + elasticHost + ':' + elasticPort + '/' + statsdIndex + '/' + elasticIndexType;
